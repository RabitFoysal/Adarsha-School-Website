...
type Staff = { id: number; name: string; designation: string; image: string; };
...
export default function ManageStaff() {
  ...
  const handleEditStart = (member: Staff) => {
    setEditId(member.id);
    setName(member.name);
  ...
  const handleAddOrUpdate = async (e: React.FormEvent) => {
    ...
    if (res.ok) {
      setMessage(editId ? "কর্মচারীর তথ্য সফলভাবে আপডেট হয়েছে!" : "নতুন কর্মচারী সফলভাবে যুক্ত হয়েছে!");
      handleCancelEdit();
      fetchStaff();
    }
    setLoading(false);
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold text-gray-800 mb-8">কর্মকর্তা ও কর্মচারী ম্যানেজমেন্ট</h1>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1 bg-white p-6 rounded-xl shadow-sm border border-gray-200 h-fit">
          <h2 className="text-xl font-bold text-gray-800 mb-6 border-b pb-4">{editId ? "✍️ তথ্য এডিট করুন" : "নতুন কর্মচারী যুক্ত করুন"}</h2>
        ...
        <div className="lg:col-span-2 bg-white p-6 rounded-xl border border-gray-200 shadow-sm"><h2 className="text-xl font-bold text-gray-800 mb-6 border-b pb-4">বর্তমান কর্মচারীদের তালিকা</h2><div className="space-y-3">{staff.map((s) => (<div key={s.id} className="p-4 rounded-xl border border-gray-150 hover:bg-gray-50 transition flex justify-between items-center gap-4"><div className="flex items-center gap-4"><img src={s.image} alt={s.name} className="w-12 h-12 rounded-full object-cover border" /><div><h4 className="font-bold text-gray-800">{s.name}</h4><p className="text-sm text-gray-500">{s.designation}</p></div></div><div className="flex gap-2"><button onClick={() => handleEditStart(s)} className="text-blue-500 hover:text-blue-700 bg-blue-50 hover:bg-blue-100 px-3 py-1.5 rounded-md text-xs font-semibold">এডিট</button><button onClick={() => handleDelete(s.id)} className="text-red-500 hover:text-red-700 bg-red-50 hover:bg-red-100 px-3 py-1.5 rounded-md text-xs font-semibold">ডিলিট</button></div></div>))}</div></div>
      </div>
    </div>
  );
}